...
def test_ranking_to_pairwise_ballots():
    ...
    assert set(ranking_to_pairwise_ballots([1, 2, 3])) == {(1,2, "win"), (1,3,"win"), (2,3,'win')}
    assert set(ranking_to_pairwise_ballots([1, {2, 3}])) == {(1,2, "win"), (1,3,"win"), (2,3,'tie')}
...
def test_pairwise_ballots_roundtrip(ranking):
    ...
    pairwise_ranked_pairs = list(nx.topological_sort(PairwiseBallotBox(pairwise).get_victory_graph()))
